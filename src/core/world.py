...
class World:
    ...
    def reset_population(
        self,
        prey_genomes: List[PreyGenome] = None,
        pred_genomes: List[PredatorGenome] = None,
    ):
    ...
    def end_generation(self):
        ...
        reproducing_predators = [
            p for p in self.predators if p.alive and p.can_reproduce()
        ]
        ...
        prey_fitness_avg = (
            sum(prey_fitness) / len(prey_fitness) if prey_fitness else 0.0
        )
        pred_fitness_avg = (
            sum(pred_fitness) / len(pred_fitness) if pred_fitness else 0.0
        )
        ...
        if reproduction_ready_prey:
            ...
            self.prey_genomes = self.evolution_engine.evolve_prey_population(
                reproduction_ready_prey,
                reproduction_fitness,
                self.params.population_prey,
                self.params,
            )
            ...
            surviving_prey_fitness = [
                self.fitness_calc.prey_fitness(p) for p in self.preys if p.alive
            ]
            self.prey_genomes = self.evolution_engine.evolve_prey_population(
                surviving_prey_genomes,
                surviving_prey_fitness,
                self.params.population_prey,
                self.params,
            )
        ...
        if reproduction_ready_pred:
            ...
            self.pred_genomes = self.evolution_engine.evolve_predator_population(
                reproduction_ready_pred,
                reproduction_fitness,
                self.params.population_pred,
                self.params,
            )
        elif len([p for p in self.predators if p.alive]) > 0:
            # Fallback to traditional fitness if no reproduction occurred
            surviving_pred_genomes = [p.dna for p in self.predators if p.alive]
            surviving_pred_fitness = [
                self.fitness_calc.pred_fitness(p) for p in self.predators if p.alive
            ]
            self.pred_genomes = self.evolution_engine.evolve_predator_population(
                surviving_pred_genomes,
                surviving_pred_fitness,
                self.params.population_pred,
                self.params,
            )
