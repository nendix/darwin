...
class Predator(BaseAgent):
    ...
    def step(self, w: int, h: int, preys: List['Prey'], all_predators: List['Predator'], params):
        ...
        if self.alive:
            ...
            for other_predator in all_predators:
                if other_predator != self and other_predator.alive:
                    if check_collision(self.pos(), other_predator.pos(), min_distance=12.0):
                        self.x, self.y = resolve_collision(self.pos(), other_predator.pos(), min_distance=12.0)
            ...
            for prey in preys:
                if prey.alive:
                    if check_collision(self.pos(), prey.pos(), min_distance=8.0):
                        ...
                        actual_resistance = prey.dna.resistance / 100.0 * 1.0  # 0-1 range for resistance
                        ...
                        if random.random() < base:
                            ...
                            self.x, self.y = resolve_collision(self.pos(), prey.pos(), min_distance=8.0)
                            self.x, self.y = clamp_to_bounds(self.x, self.y, w, h, radius=6.0)
