...
class Entity:
    ...
    def random_walk(self, dt: float, turn_probability: float = 0.1, max_turn: float = 1.0):
        """Perform random walk movement"""
        if random.random() < turn_probability:
            self.direction += random.uniform(-max_turn, max_turn)

    def find_closest_visible(self, entities: List, entity_type: type) -> Optional['Entity']:
