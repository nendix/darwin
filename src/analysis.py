...
class StatisticsAnalyzer:
    ...
    def generate_comprehensive_report(statistics: Dict[str, Any], output_dir: str = "reports") -> str:
        ...
        # Generate the three required graphs (without timestamp)
        pop_graph = StatisticsAnalyzer._create_population_graph(statistics, output_dir)
        predator_genome_graph = StatisticsAnalyzer._create_predator_genome_graph(statistics, output_dir)
        prey_genome_graph = StatisticsAnalyzer._create_prey_genome_graph(statistics, output_dir)
    ...
    def _create_population_graph(statistics: Dict[str, Any], output_dir: str) -> str:
        ...
        if time_points and predator_counts and prey_counts:
            ...
            ax.plot(time_points, predator_counts, 'r-', label='Predatori', linewidth=3, marker='o', markersize=3)
            ax.plot(time_points, prey_counts, 'b-', label='Prede', linewidth=3, marker='s', markersize=3)
            
            # Styling
            ax.set_xlabel('Tempo (secondi)', fontsize=14)
            ax.set_ylabel('Numero di Individui', fontsize=14)
            ax.set_title('Andamento delle Popolazioni nel Tempo', fontsize=16, fontweight='bold', color='white')
            ...
            ax.text(0.5, 0.5, 'Dati popolazione non disponibili', ha='center', va='center', 
                   transform=ax.transAxes, fontsize=16, color='white')
        
        graph_path = os.path.join(output_dir, f"population_graph.png")
    ...
    @staticmethod
    def _create_predator_genome_graph(statistics: Dict[str, Any], output_dir: str) -> str:
        ...
        if predator_stats:
            ...
            bars = ax.bar(traits, values, color=['#ff6b6b', '#ff8787', '#ffa8a8', '#ffc9c9'], 
            ...
            ax.set_title('Genoma Medio dei Predatori', fontsize=16, fontweight='bold', color='white')
            ax.tick_params(colors='white')
            ax.grid(True, alpha=0.3, axis='y')
        else:
            ax.text(0.5, 0.5, 'Dati genoma predatori non disponibili', ha='center', va='center', 
                   transform=ax.transAxes, fontsize=16, color='white')
        
        graph_path = os.path.join(output_dir, f"predator_genome.png")
        plt.savefig(graph_path, dpi=150, bbox_inches='tight', facecolor='#1a1a1a')
        plt.close()
    ...
    def _create_prey_genome_graph(statistics: Dict[str, Any], output_dir: str) -> str:
        ...
        if prey_stats:
            ...
            bars = ax.bar(traits, values, color=['#4dabf7', '#74c0fc', '#a5d8ff', '#d0ebff'], 
                         edgecolor='white', linewidth=1.5)
            ...
            ax.set_ylim(0, 100)
            ax.set_ylabel('Valore Medio', fontsize=14, color='white')
            ax.set_title('Genoma Medio delle Prede', fontsize=16, fontweight='bold', color='white')
            ax.tick_params(colors='white')
            ax.grid(True, alpha=0.3, axis='y')
        else:
            ax.text(0.5, 0.5, 'Dati genoma prede non disponibili', ha='center', va='center', 
                   transform=ax.transAxes, fontsize=16, color='white')
        
        graph_path = os.path.join(output_dir, f"prey_genome.png")
        plt.savefig(graph_path, dpi=150, bbox_inches='tight', facecolor='#1a1a1a')
        plt.close()
