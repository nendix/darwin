...
def create_best_prey_genome_chart(world: World, output_dir: str = "output"):
    ...
    fig.suptitle(
        "Average Prey Genome Evolution", fontsize=16, fontweight="bold", y=0.95
    )
    plt.tight_layout()
...
def create_best_predator_genome_chart(world: World, output_dir: str = "output"):
    ...
    fig.suptitle(
        "Average Predator Genome Evolution", fontsize=16, fontweight="bold", y=0.95
    )
    plt.tight_layout()
...
def create_genome_comparison_chart(world: World, output_dir: str = "output"):
    ...
    prey_traits = {
        "Speed": (prey_genome.speed - params.speed_min)
        / (params.speed_max - params.speed_min),
        "Vision": (prey_genome.vision - params.vision_min)
        / (params.vision_max - params.vision_min),
        "Stamina": (prey_genome.stamina - params.stamina_min)
        / (params.stamina_max - params.stamina_min),
        "Resistance": (prey_genome.resistance - params.resist_min)
        / (params.resist_max - params.resist_min),
    }

    # Normalize predator traits (0-1 scale)
    pred_traits = {
        "Speed": (pred_genome.speed - params.speed_min)
        / (params.speed_max - params.speed_min),
        "Vision": (pred_genome.vision - params.vision_min)
        / (params.vision_max - params.vision_min),
        "Stamina": (pred_genome.stamina - params.stamina_min)
        / (params.stamina_max - params.stamina_min),
        "Strength": (pred_genome.strength - params.strength_min)
        / (params.strength_max - params.strength_min),
    }
    ...
    bars1 = ax1.bar(
        traits, values, color=colors, alpha=0.7, edgecolor="black", linewidth=1
    )
    ax1.set_title(
        "Average Prey Genome (Final Generation)", fontsize=14, fontweight="bold"
    )
    ...
    ax2.set_title(
        "Average Predator Genome (Final Generation)", fontsize=14, fontweight="bold"
    )
