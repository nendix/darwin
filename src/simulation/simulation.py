...
class Simulation:
    ...
    def _spawn_food(self):
        ...
        current_food = len([e for e in self.entities if isinstance(e, Food) and e.alive])
    ...
    def _record_population_data(self):
        ...
        predator_count = len([e for e in self.entities if isinstance(e, Predator) and e.alive])
        ...
        self.population_history['time'].append(self.params['duration'] - self.time_remaining)
    ...
    def increase_speed(self):
        ...
        self.speed = min(MAX_SIMULATION_SPEED, self.speed + 1)
    
    def decrease_speed(self):
        """Decrease simulation speed"""
        self.speed = max(MIN_SIMULATION_SPEED, self.speed - 1)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        predator_survival_rate = (predator_count / initial_predators) * 100 if initial_predators > 0 else 0
        prey_survival_rate = (prey_count / initial_prey) * 100 if initial_prey > 0 else 0
    ...
    def _calculate_genome_statistics(self, species: str) -> Dict[str, float]:
        ...
        if species == 'predator':
            ...
            avg_vision = sum(e.genome.vision for e in entities) / len(entities)
            avg_stamina = sum(e.genome.stamina for e in entities) / len(entities)
            avg_resistance = sum(e.genome.attack_resistance for e in entities) / len(entities)
...
class PopulationManager:
    ...
    @staticmethod
    def check_reproduction_opportunities(entities: List[Entity]) -> List[Tuple[Entity, Entity]]:
        ...
        for i, predator1 in enumerate(predators):
            for predator2 in predators[i+1:]:
                if predator1.distance_to(predator2) <= PREDATOR_RADIUS * 2:
        ...
        for i, prey1 in enumerate(prey):
            for prey2 in prey[i+1:]:
                if prey1.distance_to(prey2) <= PREY_RADIUS * 2:
                    reproduction_pairs.append((prey1, prey2))
        
        return reproduction_pairs
    
    @staticmethod
    def handle_reproduction(parent1: Entity, parent2: Entity, entities: List[Entity]) -> bool:
